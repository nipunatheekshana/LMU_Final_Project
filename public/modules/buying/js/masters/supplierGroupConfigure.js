console.log('supplierGroupConfigure .js loadimng');
var parent_url = '/buying/supplierGroup_list'
$(document).ready(function () {


    $('#btnSave').on('click', function () {
        var form = $('#frmsupplierGroupConfigure ').get(0);
...
function save(data) {
    $.ajax({
        type: "POST",
        url: "/buying/supplierGroupConfigure/save",
        data: data,
        headers: {
...
function update(data) {
    $.ajax({
        type: "POST",
        url: "/buying/supplierGroupConfigure/update",
        data: data,
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
...
function loadParentGroups() {
    $.ajax({
        type: 'GET',
        url: '/buying/supplierGroupConfigure/loadParentGroups',
        async: false,
        success: function (response) {
            console.log(response)
            if (response.success) {
...
function loadSupplierGroup() {
    ...
    if (id) {
        $.ajax({
            type: "GET",
            url: "/buying/supplierGroupConfigure/loadSupplierGroup/" + id,
            processData: false,
            contentType: false,
            cache: false,
            timeout: 800000,
